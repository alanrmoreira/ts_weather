...
const App = (): JSX.Element => {
  ...
  const fetchWeather = (lat: number, lon: number, cityName: string | null = null): void => {
  ...
  `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&daily=${dailyParam},weathercode&hourly=${hourlyParam},weathercode&timezone=auto`
      ...
      .then((json) => {
        ...
        const hourlyData: HourlyForecast[] = hourlyTimes.map((t: string, i: number) => ({
          date: t.split("T")[0],
        ...
        const result: DailyForecast[] = days.map((day: string, idx: number) => {
          const hourly = hourlyData.filter(
            (h) => h.date === day && parseInt(h.hour) >= 9 && parseInt(h.hour) <= 13
          );
          return {
